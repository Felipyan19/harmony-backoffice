...
import { Bell, Building2, Inbox, LayoutDashboard, LogOut, Menu, ShieldCheck, Users, X } from 'lucide-react';
...
export function BackofficeShell({
  ...
  const renderSidebarContent = ({ onNavigate, onClose }: { onNavigate?: () => void; onClose?: () => void } = {}) => (
    <>
      <div className="flex min-h-16 items-center gap-3 border-b border-white/[0.07] px-4 py-3">
        ...
        {onClose ? (
          <button
            ...
            className="grid h-9 w-9 shrink-0 place-items-center rounded-md text-white/55 transition hover:bg-white/[0.08] hover:text-white"
          >
            <X size={19} />
          </button>
        ) : null}
  ...
  return (
    <main style={style} className="flex h-screen overflow-hidden bg-[#f3f2ee] text-neutral">
      <aside className="hidden h-screen w-[220px] shrink-0 flex-col border-r border-white/[0.07] bg-[var(--color-primary-dark)] text-white xl:flex">
      ...
      {mobileMenuOpen ? (
        <div className="fixed inset-0 z-50 xl:hidden">
          ...
          <aside
            ...
            className="relative z-10 flex h-full w-[min(84vw,280px)] flex-col border-r border-white/[0.07] bg-[var(--color-primary-dark)] text-white shadow-2xl"
          >
            {renderSidebarContent({
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-neutral/15 bg-white px-3 sm:px-5 md:px-7">
          <div className="flex min-w-0 items-center gap-3">
            <button
              ...
              className="grid h-9 w-9 shrink-0 place-items-center rounded-md text-neutral/60 transition hover:bg-neutral/5 hover:text-neutral focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/12 xl:hidden"
            >
              <Menu size={20} />
            </button>
            <div className="min-w-0">
              <h1 className="truncate text-lg font-semibold tracking-[-0.02em]">{pageMeta.title}</h1>
              <p className="mt-0.5 hidden truncate text-sm text-neutral/60 sm:block">{pageMeta.subtitle}</p>
            </div>
          </div>
...
function SidebarLink({ href, active, icon, label, badge = 0, onNavigate }: { href: string; active: boolean; icon: ReactNode; label: string; badge?: number; onNavigate?: () => void }) {
  return (
    <Link href={href} onClick={onNavigate} className={`relative flex h-10 w-full items-center gap-2.5 rounded-md px-2.5 text-left text-base font-medium transition ${active ? 'bg-white/[0.09] text-white' : 'text-white/55 hover:bg-white/[0.05] hover:text-white/90'}`}>
